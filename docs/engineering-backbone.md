...
| **Auto-renewal / negative-option** (CA ARL, FTC "click-to-cancel", state ARLs) | You bill an **auto-renewing subscription or negative-option** plan — recurring membership, free-trial-to-paid, or "subscribe & save" (physical *or* digital). | **Clear and conspicuous disclosure** of price + renewal cadence + cancellation terms before purchase; **affirmative consent** to the recurring charge (no pre-checked boxes); an **easy, symmetric cancel** — as simple to stop as to start — plus renewal/price-change reminders where required. | Disclosure + consent + easy-cancel live **before charging any recurring plan**. |
...
**B. Physical goods, consumer-product, services & funds-handling regimes** — apply per what the venture makes, sells, serves, moves, **or how it staffs and operates**. For a non-software venture these may be the *only* regimes that apply. Identify the venture's real regulatory surface and apply each that matches; treat product-liability insurance as a launch dependency, not paperwork.
...
| **⭐ Worker classification** (IC vs. employee) | The venture engages **workers as independent contractors, 1099/gig providers, or a labor marketplace** instead of W-2 employees — **the #1 legal risk for any gig/marketplace or labor-based services venture; treat as a hard dependency.** | Run a **classification analysis** against the governing tests — federal **FLSA** (economic-realities) *and* the strictest applicable **state** test (**ABC test / CA AB5** and state variants) — with **counsel sign-off**; paper the relationship with proper **independent-contractor agreements** (→ `legal-pack`) and preserve genuine independence (control, profit/loss opportunity, multiple clients); never misclassify to avoid payroll tax, benefits, or wage-and-hour obligations. | Classification analysis + counsel sign-off **before engaging any worker / provider**. |
| **Background checks / FCRA** | The venture **vets providers, contractors, or employees using consumer reports** — criminal, driving, credit, or identity checks, run directly or through a screening vendor. | Follow the **FCRA flow**: standalone written **disclosure + authorization**, then **pre-adverse- and adverse-action** notices with a copy of the report + the summary of rights; use a **compliant consumer-reporting-agency (CRA) vendor**; honor stricter **state/local + "ban-the-box"** rules. | Compliant FCRA disclosure / consent / adverse-action flow **before running any background check**. |
| **Environmental** (EPA + state/local) | The venture makes a **physical product**, handles **food**, or delivers a **physical service** that generates waste, wastewater/discharge, or emissions, or uses/stores regulated chemicals — e.g. cleaning/detailing runoff, disposal, hazmat, refrigerants. | Identify the activity's obligations and comply **per the activity** — proper waste + chemical handling and disposal, wastewater/stormwater and air rules, any required environmental permits, and safe-handling (SDS) documentation; push supplier/disposal obligations into the contract. | Environmental obligations identified + in compliance **before operating**. |
...
- [ ] Read the venture's regulatory surface for **workforce, subscription & environmental** exposure too: if it **engages workers/providers** → worker-classification analysis + counsel sign-off, and the FCRA background-check flow, before onboarding; if it **auto-renews** → disclosure + consent + easy-cancel before the first recurring charge; if it runs **physical operations** → environmental (EPA/local) compliance before operating. `[VENTURE]`
