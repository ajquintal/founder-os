...
- **v1.0 — all 15 domains built** and put through an adversarial eval + a cross-module consistency/QA pass (catalog counts reconciled to 34; `fable-brief` completeness hole for domain 7 closed; starter made chart-token-ready; PCI-DSS regime added; legal contract library de-SaaS'd; **build-shape de-SaaS'd** — domains 2/3/15 + `venture-bootstrap`/`product-spec`/`ship` now open with an explicit buy-vs-build-vs-none decision (default: off-the-shelf/no-code) and frame `starters/saas` as the custom-software archetype, one branch, not the baked-in path — scope note: the §9 compliance-regime matrix and the finance/metrics money defaults remain SaaS-shaped and are pending an industry-agnostic pass). **Next:** run it end-to-end on a real venture (human-gated accounts/entity/compliance) + install as a plugin in Claude Code.
