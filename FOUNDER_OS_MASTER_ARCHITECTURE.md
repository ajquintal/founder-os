...
- **v1.0 — all 15 domains built** and put through an adversarial eval + a cross-module consistency/QA pass (catalog counts reconciled to 34; `fable-brief` completeness hole for domain 7 closed; starter made chart-token-ready; PCI-DSS regime added; legal contract library de-SaaS'd; **build-shape de-SaaS'd** — domains 2/3 + `venture-bootstrap`/`product-spec`/`ship` now open with an explicit buy-vs-build-vs-none decision (default: off-the-shelf/no-code) and frame `starters/saas` as the custom-software archetype, one branch, not the baked-in path; **money-model + compliance de-SaaS'd** — `finance-ops` and `metrics-dashboard` now select the archetype's money model (subscription/goods/services/marketplace, never defaulted to MRR) and `docs/engineering-backbone.md §9` carries a physical-goods/food/services/funds-handling regime table (B), not a SaaS-only matrix — scope note (residual industry-agnostic work, pending): `analytics-stack` still hard-codes the single-sided AARRR funnel + an MRR/NRR money strip without the archetype guard its siblings now carry; `finance-ops`/`metrics-dashboard` still lack a marketplace/services worked example + a marketplace monthly-close step; §9 still lacks worker-classification, environmental-operations, FCRA, and auto-renewal/negative-option regimes; and domain 15 (`fable-brief`) does not yet open with the buy-vs-build-vs-none branch). **Next:** run it end-to-end on a real venture (human-gated accounts/entity/compliance) + install as a plugin in Claude Code.
