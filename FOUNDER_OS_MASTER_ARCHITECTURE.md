...
| 2 | **Product & Build** | buy-vs-build-vs-none decision first (default: the cheapest thing that ships), then the sellable product realized + validated — off-the-shelf/no-code, physical product, or service delivery, **OR** custom software (spec → deployed, tested app) | `product-spec` `venture-bootstrap` `ship` + `starters/saas` (the custom-software archetype) + dev subagents | off-the-shelf platform / no-code, else Supabase + Vercel MCP | ✅ |
| 3 | **Engineering & Infra backbone** | when building custom: stack, environments, config, migrations, performance; when buying off-the-shelf: vendor selection/config, integrations, data ownership + export | `starters/saas` + `docs/engineering-backbone.md` (custom build); vendor config (off-the-shelf) | Supabase, Vercel, GitHub, else platform admin | ✅ |
...
| 15 | **Fable-Ready Build Spec** | comprehensive PR-by-PR build brief (scope, data model, acceptance criteria, tests, security, gates) — code PRs when building custom, config/ops PRs (vendor setup · automations · manual-QA / one-real-transaction acceptance) when buying/assembling off-the-shelf | `fable-brief` skill (capstone) | consumes 1–14 | ✅ |
...
- **Compliance-by-design:** the applicable regimes — read from the venture's regulatory surface (data/privacy, health, product-safety/liability, funds-handling, worker-classification, environmental, advertising/claims, auto-renewal), never assumed to be a software default — mapped to concrete controls before launch, not after, per `docs/engineering-backbone.md §9`'s two matrices.
...
Detailed in `docs/tool-mcp-stack.md`. Anchor mapping — the **product/commerce/payments spine is archetype-conditional** (software → Supabase + Vercel + Stripe; commerce/goods → a storefront like Shopify + Shopify Payments/Stripe; marketplace → Stripe Connect; retail/hospitality → Square/Toast POS; services → a booking/dispatch tool — the archetype tool layer in the doc), while the back office is shared: **CRM/sales + ops tables** → Airtable (MCP + sales-ops/product-ops/marketing-ops skills). **Comms** → Twilio (SMS/voice) + SendGrid (email). **Design** → Canva (MCP). **Docs/knowledge** → Google Drive/Gmail (MCPs). **Diagrams/orgcharts** → Lucid (MCP). **Analytics** → GA4 + PostHog. **Finance** → Stripe + accounting connector + `finance:*` skills. Anything missing is found via `SearchMcpRegistry` and surfaced to Tony to connect.
...
- **6–48h Build:** venture-bootstrap runs the buy-vs-build-vs-none decision first — default to configuring off-the-shelf tools (storefront, booking, POS) and skip the codebase entirely; only clone `starters/saas` and run product-spec → `/ship` when the venture genuinely needs custom software, with the security/RBAC backbone (3,4) baked in from the starter. Finance (9) + CRM (8) + analytics (13) stood up on their tools/MCPs in parallel.
...
- **Built (wave 2):** domain 10 — `legal-pack` (the legal, entity & compliance foundation: entity choice by goal — LLC/C-corp/S-corp — + operating agreement/bylaws + cap-table basics; the core contract set as review-flagged draft templates — ToS/privacy/DPA/NDA/MSA/SOW/contractor/employment/referral + model-specific goods/retail (lease/supply/sale-of-goods/IP-license); IP assignment + trademark path, flag-never-clear; the compliance regime by industry — GDPR/CCPA · HIPAA · LegitScript · licensure · SOC2 — pulled from engineering-backbone §9; and the templatable-vs-lawyer split. Composes `setup-checklists` + the `legal:*` pack. **NOT legal advice — every judgment point flags attorney review; drafts only, never files or signs.** Consumed by `fable-brief` as workstream H).
- **v1.0 — all 15 domains built** and put through an adversarial eval + a cross-module consistency/QA pass (catalog counts reconciled to 34; `fable-brief` completeness hole for domain 7 closed; starter made chart-token-ready; PCI-DSS regime added; legal contract library de-SaaS'd; **build-shape de-SaaS'd** — domains 2/3 + `venture-bootstrap`/`product-spec`/`ship` now open with an explicit buy-vs-build-vs-none decision (default: off-the-shelf/no-code) and frame `starters/saas` as the custom-software archetype, one branch, not the baked-in path; **domain 15 (`fable-brief`) too** — it now sequences on an archetype-selected, compliance-first phase/go-live order (never the SaaS default) and carries operational workstreams as first-class config-PRs; **money-model + compliance de-SaaS'd** — `finance-ops`, `metrics-dashboard`, **and `analytics-stack`** now read the venture's archetype (subscription/goods/services/marketplace, never defaulted to MRR): the finance money model, the metric-tree money strip, and analytics-stack's funnel + money strip are all archetype-conditional; and `docs/engineering-backbone.md §9` carries the physical-goods/food/services/funds-handling regimes **plus worker-classification, FCRA/background-check, environmental, auto-renewal/negative-option, and FTC advertising/Green-Guides regimes** — two matrices read from the venture's regulatory surface, not a SaaS-only list; **`docs/tool-mcp-stack.md`** now carries an archetype tool layer (commerce/DTC, marketplace payments via Stripe Connect, 3PL/inventory/fulfillment, POS, booking/dispatch) so the stack isn't SaaS-web-app-only — scope note (one residual pending): the money skills still lack a **marketplace/services worked example** (`finance-ops` ships subscription + goods examples, `metrics-dashboard` only subscription) plus a marketplace monthly-close step). **Next:** run it end-to-end on a real venture (human-gated accounts/entity/compliance) + install as a plugin in Claude Code.
