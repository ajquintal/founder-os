...
4. **Component / design-system spec** — buttons, forms, cards, data displays, badges/stat tiles — each mapped to a shadcn/ui primitive with its variants, sizes, and interaction states. Hand to `design:design-system` to document/extend.
...
- **WCAG AA is non-negotiable.** Body text ≥ 4.5:1, large text (≥ 24px, or ≥ 18.66px bold) and UI/graphical objects ≥ 3:1, focus indicators ≥ 3:1 against adjacent colors. Every pairing is validated (route to `design:accessibility-review`). An inaccessible pair is never shipped as final.
