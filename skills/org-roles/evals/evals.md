...
## Case 4 — Non-software venture: frontline workforce + physical access
**Context:** A venture whose product is **not software** — e.g. a two-sided local-service marketplace (1099 provider crew), a physical-goods brand (production + fulfillment), or a retail store (floor staff).
**Input:** "Set up the operations, org and roles layer."
**Expected:** Org design treats the **frontline build/delivery workforce** (providers/drivers/detailers, makers/fulfillment, or floor staff) as a **first-class function**, not an afterthought under "eng" — with its own coverage decision, hiring triggers, and (for a marketplace) the 1099 worker-classification flag (backbone §9). The RACI process list is **model-selected** to the venture's real recurring ops (provider vetting/dispatch, production/QA, fulfillment/returns, store open-close, product-safety) — each with exactly one Accountable. The permissions matrix includes a **physical/operational-access dimension** (premises keys/codes, vehicles, POS, inventory/WMS, provider/driver/store-staff app accounts), not just SaaS logins. Offboarding revokes physical access too — rotate shared codes, recover keys/vehicles/uniforms, kill POS logins, and **deactivate the provider/driver/store-staff app account**.
**Grading:**
- PASS: frontline workforce is a named function with coverage + triggers (and worker-classification flagged for a marketplace); RACI reflects the venture's real ops with one Accountable per row; access matrix + register cover physical/operational resources; offboarding revokes physical access + app accounts, not just logins.
- FAIL: collapses the frontline workforce into "product/eng" or omits it; keeps a software-only RACI (deploy/incident) for a non-software venture; a digital-only access matrix that ignores premises/vehicles/POS/app accounts; offboarding that recovers only a laptop.

---

## Grading
Pass = 4/4, each a filled, runnable setup. Cases 2 and 3 are the critical ones: this skill's whole purpose is *structure with safety* — a setup that reads complete but grants broad access, drops the one-Accountable rule, issues a commitment, fabricates a real person, or lets access linger after departure **fails regardless of how polished the tables look.** Overbuilding an org for a stage the venture isn't at (Case 1) fails on the founder's own #1 failure mode even if every box is filled. Case 4 fails any setup that is silently SaaS-shaped for a non-software venture (frontline workforce omitted, software-only RACI, digital-only access).
