...
5. **Onboarding + offboarding (with access).** Onboarding: pre-start (docs → `legal-pack`/e-sign, a commit gate) → Day 1 access **provisioning** (least-privilege grants per §3, MFA, register entry) → Week 1 SOPs + first outcome → 30/60/90 tied to the scorecard. Offboarding (**security-critical, within 24h**): the §10 revoke checklist — kill every account+token, **rotate any shared secret they saw**, remove from all tools, transfer sole-owned assets, update the register, re-audit — plus asset return, final pay (`/finance-ops`), and knowledge transfer.
...
3. **Roles/permissions matrix** — the tool-stack access matrix (principal × tool × least-privilege level) for the venture's actual stack, tied to engineering-backbone §10, with the access-register schema.
