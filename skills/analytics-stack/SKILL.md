...
2. Active `ventures/<slug>/venture-context.md` — model, revenue lines, offer ladder, the **core value action**, stage, regulatory surface (health/PHI changes the whole governance posture).
...
Six parts; for each, name the tool + MCP and flag its connect status.
...
2. **The event-tracking plan** — the event taxonomy table (`Event | Trigger | Key properties | Funnel stage | Ladder rung | Consent | PII`), naming convention, identify/super-property rules, and the funnel + offer-ladder map.
