...
7. **Sequence + parallelize.** Phase the PRs (Phase 0 foundation → 1 core value → 2 payment/launch surfaces → 3 operate), draw the dependency graph (what unblocks what), mark what runs in parallel, and name the **critical path to first dollar** + the go-live gate sequence. **The go-live sequence is archetype-conditional and compliance-first:** compliance / legal / safety / licensing / insurance / worker-classification are HARD PREREQUISITE gates that MUST clear *before* any real customer transaction and *before* any liquidity / growth / traction gate — **no real paid transaction before the venture is legally cleared to transact; a growth/liquidity gate can never precede the compliance/insurance/licensing gate.** For custom-software the payment path is `deploy → test-mode payments → payments go-live`; for services / marketplace / goods it is `legal/insurance/licensing clearance → operational readiness → first real transaction → scale` (for non-software ventures "test mode" is a payment-plumbing dry-run, never real paid customer work). Each gate is human-approved.
...
2. **Sequencing & parallelization** — the phase plan + dependency graph + critical path to first dollar + the go-live gate sequence (archetype-conditional and compliance-first: the compliance/insurance/licensing gate clears before any real transaction or liquidity/growth gate).
...
- **Compliance clears before commerce (hard ordering).** Compliance / legal / safety / licensing / insurance / worker-classification are HARD PREREQUISITE gates: no real paid customer transaction — and no liquidity / growth / traction gate — is ever sequenced before the venture is legally cleared to transact. **A growth/liquidity gate can never precede the compliance/insurance/licensing gate.** The go-live order is archetype-conditional (custom-software: deploy → test-mode payments → payments go-live; services/marketplace/goods: legal/insurance/licensing clearance → operational readiness → first real transaction → scale), and for non-software ventures "test mode" is never real paid customer work.
